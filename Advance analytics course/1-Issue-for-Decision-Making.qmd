...
Summarizes the overall variability of the log-transformed data. It is computed as the average of the diagonal elements (variances) of the log-transformed covariance matrix:

$[
\text{GV} = \frac{1}{p} \text{tr}(\Sigma_{\text{log}})
]$

where:

-   $( \text{tr}(\Sigma_{\text{log}}) )$ is the trace of $( \Sigma_{\text{log}} )$ (sum of its diagonal elements).
